                        ...
                        <form method="POST" action="{{ isset($post) && $post->id ? route('post.process', ['id' => $post->id]) : route('post.process') }}" enctype="multipart/form-data">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <div id="editor"></div>
                                    <textarea style="display:none;" name="title" id="titleTextarea">{{ isset($post) ? $post->title : old('title') }}</textarea>
                                    @error('title')
...
<script>
    ClassicEditor
        ...
        .then(editor => {
            editor.setData(document.getElementById('titleTextarea').value);
            editor.model.document.on('change:data', () => {
                document.getElementById('titleTextarea').value = editor.getData();
            });
        })
        .catch(error => {
            console.error(error);
